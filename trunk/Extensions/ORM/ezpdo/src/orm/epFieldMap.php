...
/**
 * $Id: epFieldMap.php,v 1.15 2005/12/03 22:43:06 nauhygon Exp $
 * 
...
class epFieldMapFactory {
    ...
    static function &make($name, $type, $params = array()) {
        ...
        case epFieldMap::DT_COMPOSED_OF: {
            ...
            $i = 0; // cursor in params
            
            // get is_many
            $is_many = false;
            switch ($params[$i]) {
            
                case epFieldMapRelationship::MT_ONE: 
                    $i ++;
                    break;
            
                case epFieldMapRelationship::MT_MANY: 
                    $is_many = true;
                    $i ++;
                    break;
            }
            ...
            if (!isset($params[$i]) || !$params[$i]) {
                // throw if class related is not specified
                throw new epExceptionFieldMapFactory('Class related is not specified for variable [' . $name . '] (type: ' . $type . ')');
                break;
            }
            $class = $params[$i];
            $i ++;
            ...
            // get inverse if any
            $inverse = false;
            if (isset($params[$i]) && $params[$i] == 'inverse') {
            ...
            $fm = new epFieldMapRelationship($name, $type, $class, $is_many, $inverse);
            break;
        }
