...
/**
 * $Id: epParser.php,v 1.13 2005/12/11 18:24:26 nauhygon Exp $
 * 
 ...
 * 
 * The parser parses ({@link epParser::parse()}) a PHP source file 
 ...
 * @subpackage ezpdo.compiler
 */
class epParser extends epConfigurableWithLog {
    ...
    public function parse($file, $classes = null) { 
        ...
        while (false !== ($this->token = $this->scanner->next())) {
            ...
            if (!in_array($token_name, epParser::$tokens_to_process)) {
                continue;
            }
    ...
    protected function parseClassComment(&$cm, $comment) {
        ...
        if (!($value = $c->getTagValue('orm'))) {
            //warn('Cannot parse comment for class [' . $cm->getName() . ']');
            return true;
        }
        
        if (!($t = new epClassTag($value))) {
